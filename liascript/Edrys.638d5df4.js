...
})({"aNQw6":[function(require,module,exports) {
var global = arguments[3];
var HMR_HOST = null;
var HMR_PORT = null;
var HMR_SECURE = false;
var HMR_ENV_HASH = "d6ea1d42532a7575";
module.bundle.HMR_BUNDLE_ID = "81815947638d5df4";
"use strict";
...
},{}],"fBMQ1":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "uint8_to_base64", ()=>uint8_to_base64);
parcelHelpers.export(exports, "base64_to_unit8", ()=>base64_to_unit8);
parcelHelpers.export(exports, "Sync", ()=>Sync);
var _helper = require("../../helper");
var _db = require("./db");
var _uint8ToBase64 = require("uint8-to-base64");
function uint8_to_base64(data) {
    return (0, _uint8ToBase64.encode)(data);
}
function base64_to_unit8(data) {
    return (0, _uint8ToBase64.decode)(data);
}
...
function dynamicGossip(self) {
    ...
    function delay() {
        ...
        return (self.db.getPeers().length + 1) * 200 + (self.db.initialized ? 5000 : 1000);
    }
    function publish() {
    ...
    return ()=>{
        ...
        if (self.db.initialized) publish();
        else if (self.db.initPromise) self.db.initPromise.then(publish);
        else timerID = window.setTimeout(publish, delay());
    };
}
class Sync {
    isConnected = false;
    replyOnReceive = false;
   ...
   */ constructor(cbConnection, cbRelay, onConnect, onReceive, replyOnReceive = false, useInternalCallback = true){
        let token;
        try {
        ...
        this.gossip = gossip;
        const throttleBroadcast = _helper.throttle(()=>{
            self.broadcast(true, self.db.encode());
            gossip();
        }, 1000);
        this.db = new (0, _db.CRDT)(token, useInternalCallback ? (event, origin)=>{
            if (self.db) {
                switch(origin){
                    case "cursor":
                        this.sync("update", {
  ...
  */ /** This method shall be overwritten, and trigger the connection for the
   ...
   */ uniqueID() {
        ...
        if (typeof this.room === "string" && (this.room.startsWith('"') && this.room.endsWith('"') || this.room.startsWith("'") && this.room.endsWith("'"))) return this.room;
        // otherwise a combination of course-url and room-name are used
        if (this.course && this.room) return JSON.stringify({
            course: this.course,
            room: this.room,
            pw: _helper.getHashCode(this.password || "")
        });
   ...
   */ sendToLia(event) {
    ...
    sync(topic, message = null) {
        this.cbConnection(topic, message);
    }
    sendDisconnectError(msg) {
        this.sync("error", msg);
    }
    sendConnect() {
    ...
    pubsubSend(topic, message) {
        ...
        if (this.replyOnReceive) this.onReceive?.(topic, message);
    }
    pubsubReceive(data) {
        try {
   ...
   */ load(url, obj) {
    ...
    publish(event) {
        switch(event.message.cmd){
            ...
            case "chat":
                ...
                break;
            case "quiz":
                if (event.track?.[0][0] === "quiz" && event.track?.[1][0] === "id") this.db.addQuiz(event.track[0][1], event.track[1][1], event.message.param);
                else console.warn("SyncTX wrong event ->", event);
                break;
            case "survey":
                if (event.track?.[0][0] === "survey" && event.track?.[1][0] === "id") this.db.addSurvey(event.track[0][1], event.track[1][1], event.message.param);
                else console.warn("SyncTX wrong event ->", event);
                break;
            case "code":
                if (event.track?.[0][0] === "code" && event.track?.[1][0] === "id") this.db.updateCode(event.track[0][1], event.track[1][1], event.message.param.j, event.message.param.msg);
                else console.warn("SyncTX wrong event ->", event);
                break;
            case "codes":
                if (event.track?.[0][0] === "code" && event.track.length === 1) {
                    for(let i = 0; i < event.message.param.length; i++)for(let j = 0; j < event.message.param[i].length; j++)this.db.initCode(event.track[0][1], i, j, event.message.param[i][j]);
                } else console.warn("SyncTX wrong event ->", event);
                break;
            case "cursor":
                if (event.track?.[0][0] == "code") this.db.setCursor(event.track[0][1], event.message.param);
                break;
            case "broadcast":
    ...
    applyUpdate(data, force = false) {
        this.db.applyUpdate(data, force);
    }
}

},{"../../helper":"7hqDz","./db":"aHGFf","uint8-to-base64":"gcOzN","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"aHGFf":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "CRDT", ()=>CRDT);
...
class CRDT {
    ...
    initialized = false;
    initPromise = null;
    constructor(peerID, callback){
        this.doc = new _yjs.Doc();
        this.callback = callback || ((e, origin)=>{
        ...
        this.quizzes = new (0, _yKeyvalue.YKeyValue)(this.doc.getArray(QUIZ));
        this.surveys = new (0, _yKeyvalue.YKeyValue)(this.doc.getArray(SURVEY));
        this.chat = new (0, _yKeyvalue.YKeyValue)(this.doc.getArray("chat"));
    }
    init(data) {
    ...
    registerCallbacks() {
        ...
        this.quizzes.on("change", (changes)=>{
            ...
            if (updates) this.callback(updates, "quiz");
        });
        this.surveys.on("change", (changes)=>{
            const updates = this.getUpdates(this.surveys, changes);
            if (updates) this.callback(updates, "survey");
        });
        this.chat.on("change", (changes)=>{
            const vector = [];
            let obj;
            for (let [id, op] of changes)if (op.action === "add") {
                obj = op.newValue;
                obj["id"] = parseInt(id);
                vector.push(obj);
            }
        ...
        this.codes.observeDeep((events)=>{
            ...
            if (ids.size > 0) this.callback(this.getCode(ids), "code");
        });
        this.peers.set(this.peerID, true);
    }
    encode() {
        return _yjs.encodeStateAsUpdate(this.doc);
    }
    destroy() {
        this.doc.destroy();
    }
    applyUpdate(update, force = false) {
        this.doc.transact(()=>{
            if (force) {
                this.doc.destroy();
                this.doc = new _yjs.Doc();
                _yjs.applyUpdate(this.doc, update);
                this.peers = this.doc.getMap(PEERS);
                this.cursors = this.doc.getMap(CURSORS);
                this.codes = this.doc.getMap(CODE);
                this.quizzes = new (0, _yKeyvalue.YKeyValue)(this.doc.getArray(QUIZ));
            ...
            } else _yjs.applyUpdate(this.doc, update);
        });
    }
    log() {
        console.warn("*********** PEERS ***********");
    ...
    */ }
    initMap(map, id, data) {
        ...
        for(let i = 0; i < data.length; i++){
            ...
            if (!state) state = {};
            for(const key in data[i])state[key] = data[i][key];
            map.set(this.id(id, i), state);
        }
    }
    initText(id, data) {
        if (data.length === 0) return;
        for(let i = 0; i < data.length; i++)for(let j = 0; j < data[i].length; j++)this.initCode(id, i, j, data[i][j]);
    }
    diff(state) {
        return _yjs.encodeStateAsUpdate(this.doc, state);
    }
    getCode(ids) {
        let vector = [];
        for (const id of ids)vector.push({
            id: id,
            data: this.getAllTexts(id)
        });
        return vector;
    }
    getCursors(keys) {
    ...
    getPeers() {
        ...
        if (peers) return Object.entries(peers).filter(([_, value])=>value).map(([key, _])=>key);
        return [];
    }
    removePeer(peerID) {
        this.doc.transact(()=>{
        ...
        if (peerID === undefined) this.callback(this.encode(), "exit");
    }
    id(id1, id2, id3) {
        if (id3 === undefined) return JSON.stringify([
            id1,
            id2
        ]);
    ...
    getAllMaps(map) {
        ...
        for(let i = 0; i < this.length; i++){
            let sub = [];
            for(let j = 0; map.has(this.id(i, j)); j++)// @ts-ignore
    ...
    getMaps(id, map) {
        const vector = [];
        for(let i = 0; map.has(this.id(id, i)); i++)// @ts-ignore
    ...
    addSurvey(id, i, value) {
        this.addRecord(this.surveys, id, i, value);
    }
    addRecord(map, id, i, value) {
    ...
    updateCode(id, i, j, messages) {
        if (this.codes.has(this.id(id, i, j))) this.doc.transact(()=>{
            const code = this.codes.get(this.id(id, i, j));
            if (code === undefined) return;
            for (let msg of messages)switch(msg.action){
                case "insert":
    ...
    getColor() {
        if (!this.color) this.color = _helper.getColorFor(this.peerID);
        return this.color;
    }
    setCursor(section, cursor) {
        this.doc.transact(()=>{
    ...
    getUpdates(maps, changes) {
        const ids = new Set();
        const updates = [];
        for (const [id, data] of changes)switch(data.action){
            case "update":
                if (JSON.stringify(Object.keys(data.oldValue).sort()) !== JSON.stringify(Object.keys(data.newValue).sort())) {
                    updates.push([
                        id,
                        {
                            ...data.oldValue,
        ...
        for (const id of ids)vector.push({
            id: id,
            data: this.getMaps(id, maps)
        });
        for (const [id, value] of updates)maps.set(id, value);
        if (vector.length > 0) return vector;
        return null;
    }
}

},{"yjs":"gj9M9","../../helper":"7hqDz","y-utility/y-keyvalue":"204vy","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"gj9M9":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
var Buffer = require("d7b07e8b3779e46d").Buffer;
...
},{"d7b07e8b3779e46d":"fCgem","./string.js":"cA9Az","./environment.js":"h8wYT","./array.js":"ef0AT","./math.js":"jhXHz","./encoding.js":"9yVNx","./decoding.js":"ftn2I","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"h8wYT":[function(require,module,exports) {
/**
...
var process = require("97948f306b79c760");
...
},{"97948f306b79c760":"d5jf4","./map.js":"eE1vX","./string.js":"cA9Az","./conditions.js":"dJDk6","./storage.js":"9FcRV","./function.js":"1O0Qj","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"dJDk6":[function(require,module,exports) {
/**
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"204vy":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
/**
 * @template T
 ...
 * })
 * ```
 */ parcelHelpers.export(exports, "YKeyValue", ()=>YKeyValue);
var _yjs = require("yjs"); // eslint-disable-line
var _observable = require("lib0/observable");
class YKeyValue extends (0, _observable.Observable) {
    /**
     ...
     */ this.map = new Map();
        {
            ...
            this.doc.transact(()=>{
                for(let i = arr.length - 1; i >= 0; i--){
                    const v = arr[i];
                    if (this.map.has(v.key)) // entry already exists, remove the current item
                    yarray.delete(i);
                    else this.map.set(v.key, v);
                }
            });
        }
         ...
         */ for(let i = vals.length - 1; i >= 0 && (addedVals.size > 0 || itemsToRemove.size > 0); i--){
                    ...
                    if (itemsToRemove.has(currVal.key)) {
                        ...
                        if (prevValue) {
                            ...
                            if (delEvent && delEvent.action === "delete") changes.set(currVal.key, {
                                action: "update",
                                newValue: currVal.val,
                                oldValue: delEvent.oldValue
                            });
                            else // fire an "add" event
                            changes.set(currVal.key, {
                                action: "add",
                                newValue: currVal.val
                            });
            ...
            if (changes.size > 0) this.emit("change", [
                changes,
                tr
            ]);
        });
...
},{"yjs":"gj9M9","lib0/observable":"20Rfd","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"gcOzN":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "encode", ()=>encode);
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["aNQw6"], null, "parcelRequire55a5")

//# sourceMappingURL=Edrys.638d5df4.js.map
