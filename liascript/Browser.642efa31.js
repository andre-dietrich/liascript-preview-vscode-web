...
})({"finfG":[function(require,module,exports) {
var global = arguments[3];
var HMR_HOST = null;
var HMR_PORT = null;
var HMR_SECURE = false;
var HMR_ENV_HASH = "d6ea1d42532a7575";
module.bundle.HMR_BUNDLE_ID = "35ab1e24642efa31";
"use strict";
...
},{}],"annGt":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "Connector", ()=>Connector);
var _log = require("../../liascript/log");
var _logDefault = parcelHelpers.interopDefault(_log);
var _database = require("./database");
var _index = require("../Base/index");
class Connector extends _index.Connector {
    constructor(){
        super();
        this.database = new _database.LiaDB();
    }
    hasIndex() {
        return true;
    }
    async open(uidDB, versionDB, slide) {
    ...
    async reset(uidDB, versionDB) {
        if (uidDB && versionDB) {
            ...
            (0, _logDefault.default).info("DB: reset => ", uidDB, versionDB);
        }
    }
    getFromIndex(uidDB) {
        return this.database.getIndex(uidDB);
    }
    async addMisc(uidDB, versionDB, key, value) {
        this.database.addMisc(uidDB, versionDB, key, value);
    }
    async getMisc(uidDB, versionDB, key) {
        return this.database.getMisc(uidDB, versionDB, key);
    }
}

},{"../../liascript/log":"4dsMr","./database":"dzbbJ","../Base/index":"aFHaA","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"dzbbJ":[function(require,module,exports) {
// @ts-ignore
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "LiaDB", ()=>LiaDB);
var _dexie = require("dexie");
var _dexieDefault = parcelHelpers.interopDefault(_dexie);
var _log = require("../../liascript/log");
var _logDefault = parcelHelpers.interopDefault(_log);
// @ts-ignore
(0, _dexieDefault.default).debug = true;
class LiaDB {
   ...
   *
   */ constructor(){
        this.dbIndex = new (0, _dexieDefault.default)("Index");
   ...
   */ open_(uidDB) {
        let db = new (0, _dexieDefault.default)(uidDB);
        db.version(1).stores({
   ...
   */ async open(uidDB, versionDB, init) {
        ...
        try {
            ...
            (0, _logDefault.default).warn("DB: open -> ", e.message);
            this.db = null;
        }
        if (init && this.db) {
   ...
   */ async store(record, versionDB) {
        if (!this.db || this.version === 0) return;
        (0, _logDefault.default).warn(`liaDB: event(store), table(${record.table}), id(${record.id}), data(${record.data})`);
   ...
   */ async load(record, versionDB) {
        if (!this.db) return;
        (0, _logDefault.default).info("loading => ", record.table, record.id);
        const item = await this.db[record.table].get({
            id: record.id,
            version: versionDB != undefined ? versionDB : this.version
        });
        if (item) {
            (0, _logDefault.default).info("restore table", record.table);
            return item.data;
        }
        return null;
    }
   ...
   */ async slide(id) {
        try {
            ...
            (0, _logDefault.default).warn("DB: could not update slide => ", id);
        }
    }
   ...
   */ async transaction(record, modify) {
        if (!this.db || this.version === 0) return;
        let db = this.db;
   ...
   */ async getIndex(uidDB) {
        try {
            ...
            (0, _logDefault.default).warn("DB: getIndex -> ", e.message);
        }
        return null;
    }
   ...
   */ async storeIndex(data) {
        if (!this.dbIndex.isOpen()) {
            (0, _logDefault.default).warn("DB: storeIndex ... db is closed");
            return;
        }
        const date = new Date();
        ...
        if (!item.data[data.version]) {
            ...
            (0, _logDefault.default).info("storing new version to index", item);
        ...
        this.dbIndex["courses"].put(item).then(function(result) {
            (0, _logDefault.default).info("DB: storeIndex", result);
        });
    }
    async addMisc(uidDB, versionDB, key, value) {
   ...
   */ async deleteIndex(uidDB) {
        await Promise.all([
            ...
            (0, _dexieDefault.default).delete(uidDB)
        ]);
    }
...
},{"dexie":"2sctG","../../liascript/log":"4dsMr","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"2sctG":[function(require,module,exports) {
/*
...
parcelHelpers.export(exports, "PropModification", ()=>PropModification);
...
parcelHelpers.export(exports, "replacePrefix", ()=>replacePrefix);
var global = arguments[3];
...
var Collection = function() {
    ...
    Collection.prototype.delete = function() {
        ...
        if (isPlainKeyRange(ctx) && (ctx.isPrimKey || range.type === 3)) return this._write(function(trans) {
...
var Dexie$1 = function() {
    ...
    Dexie.prototype.delete = function(closeOptions) {
        var _this = this;
        if (closeOptions === void 0) closeOptions = {
            disableAutoOpen: true
        };
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"aFHaA":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
class Connector {
   ...
   */ hasIndex() {
    ...
    initSettings(data, local = false) {
        return (0, _settings.Settings).init(data, local, this.setSettings);
    }
    setSettings(data) {
        (0, _settings.Settings).update(data);
    }
    getSettings() {
        let data = "";
        try {
            data = localStorage.getItem((0, _settings.Settings).PORT);
        } catch (e) {
        ...
        if (typeof data === "string") {
            ...
            if (!json) json = (0, _settings.Settings).data;
            if (window.innerWidth <= 768) json.table_of_contents = false;
        }
        return json;
    }
    open(_uidDB, _versionDB, _slide) {}
    load(_record) {}
    store(_record) {}
    update(_record, _fn) {}
    slide(_id) {}
    ...
    async storeToIndex(_json) {}
    restoreFromIndex(_uidDB, _versionDB) {}
    reset(_uidDB, _versionDB) {
        this.initSettings(null, true);
    }
    async getFromIndex(_uidDB) {
        return null;
    }
    async addMisc(_uidDB, _versionDB, _key, _value) {
        console.log("addMisc not implemented");
    }
    async getMisc(_uidDB, _versionDB, _key) {
        console.log("getMisc not implemented");
    }
}

},{"./storage":"d4zre","./settings":"3xoes","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"d4zre":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "LiaStorage", ()=>LiaStorage);
class LiaStorage {
    constructor(){}
    getItems(key = []) {
    ...
    _setLocal(dict) {
        if (typeof dict === "object") for (const [key, value] of Object.entries(dict))localStorage.setItem(key, JSON.stringify(value));
    }
}

},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["finfG"], null, "parcelRequire55a5")

//# sourceMappingURL=Browser.642efa31.js.map
